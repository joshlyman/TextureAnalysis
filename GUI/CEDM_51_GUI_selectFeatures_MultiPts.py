# version 3: select features from multiple patients
...
featuresOutFn = 'CEDM_51_batchtest_features.csv'
...
def genFeatures(rootDir,phasename,patientslist,GLCMdist,LBPrad,GLCM,LBP,GABOR,RAW):
    ...
    with open(featuresCSVFn, 'wb') as featureCSVFile:
        ...
        # Benign/DES-CC/
        phasefolder = os.path.join(rootDir, phasename)

        for casefile in patientslist:

            lesionROIRectfile = casefile + '.csv'
            lesionfile = casefile + '.dcm'

            lesionROIRectFn = os.path.join(phasefolder,lesionROIRectfile)
            lesionDicom = os.path.join(phasefolder,lesionfile)
...
# 3. patientslist: a list of multiple patients inside after user click select patients
# 4. GLCMdist: GLCM distance
# 5. LBPrad: LBP radius
# 6. GLCM, LBP, GABOR, RAW: add/delete 4 features: True/False


# Assume this is input path from user's browser / load dataset
rootDir = '/Users/yanzhexu/Desktop/Research/TA GUI/CEDM/Batchtest/Benign/'

genFeatures(rootDir,phasename='DES-CC',patientslist=['Pt1','Pt2'],GLCMdist=1,LBPrad=1,GLCM=True,LBP=True,GABOR=True,RAW=True)
