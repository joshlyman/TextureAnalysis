...
from Gabor import MaxACRExtendGaborFeatures
...
featuresOutFn = 'ACR_MaxMap_features_LBP1.csv'
# featuresOutFn = 'ACR_MaxMap_features_LBP3.csv'
...
def genFeatures():
    ...
    Gaborkernel_bank = MaxACRExtendGaborFeatures.genKernelBank(Gaborsigma_range, Gaborfreq_range, kernel_bank)
    ...
    with open(featuresCSVFn, 'wb') as featureCSVFile:
        ...
        for ACRfolder in os.listdir(rootDir):
            ...
            for lesionfolder in os.listdir(ACRpath):
                ...
                # get subimage/ LBP subimage from dicom image of 4 time points
                ...
                # calculate raw mean and std of max image map
                mean_LargBox = numpy.mean(MaxSubImage)
                std_LargBox = numpy.std(MaxSubImage)

                # add Kurtosis and Skewness into Raw Features
                MaxSubImagelist = list()
                MaxSubImageArraylist = MaxSubImage.tolist()
                for smalllist in MaxSubImageArraylist:
                     MaxSubImagelist+= smalllist

                Kurtosis_LargBox = kurtosis(MaxSubImagelist)
                Skewness_LargBox = skew(MaxSubImagelist)

                # normalize subimage GLCM of max map
                subImageGLCM = GrayScaleNormalization(MaxSubImage, MaxSubImage.max(), MaxSubImage.min())

                #print 'GLCM:',subImageGLCM

                # for extended LBP, we still use grayscale range of 8*8 box to normalize extended ROI 10*10 box (LBP radius = 1)
                extendsubImageLBP = GrayScaleNormalization(MaxLBPSubImage, MaxSubImage.max(), MaxSubImage.min())
                ...
                # Gabor
                GaborFeatures = MaxACRExtendGaborFeatures.calcFeatures(dicomImage1,dicomImage2,dicomImage3,dicomImage4, xcoord1, ycoord1, xcoord2,
                ycoord2,xcoord3,ycoord3,xcoord4,ycoord4, width1, height1, Gaborkernel_bank, MaxSubImage.max(), MaxSubImage.min())
