...
def Norm_Mean_Std_LargestBox2(dicomimage,largeX,largeY,largeX2,largeY2):
    ...
    # normalized to [0,255]
    LargeBoxGrayLevelNorm = (LargeBoxGrayLevelNorm - ming) * (256.0 / float(maxg-ming))
...
def GrayScaleNormalization(imgArray, imgRange,imgMin):
    ...
    imgArray = (imgArray - imgMin) * (256.0 / float(imgRange))
        ...
        for slicelist in ptslice:
            ...
            for GLCMAngle in GLCMAngleList:
                for eachdcm in dicomnames:
                    ...
                    subImage = dicomImage[ycoord-4:ycoord+4,xcoord-3:xcoord+5]


                    ## get raw mean and standard deviation of lesion ROI's gray level of lagest box directly from subImage
                    dicommean, dicomstd,maxgraylevel,mingraylevel = Mean_Std_LargestBox2(dicomImage, xcoord - 3, ycoord - 4, xcoord+5, ycoord+4)
            ...
            for eachdcm in dicomnames:
                ...
                dicomImage = Read2DImage(dicomfilepath)
                subImage = dicomImage[ycoord - 2: ycoord + 6,xcoord - 2:xcoord + 6]

                ## get normalized to 0, 255: raw mean and standard deviation
                dicommean, dicomstd, maxgraylevel, mingraylevel = Norm_Mean_Std_LargestBox2(dicomImage, xcoord - 2, ycoord - 2, xcoord+6, ycoord+6)
                ...
                # LBP subimage
                subImageLBP = dicomImage[ycoord - 3 - LBPRadius:ycoord + 4 + LBPRadius,
                              xcoord - 3 - LBPRadius: xcoord + 4 + LBPRadius]

                extendsubImageLBP = GrayScaleNormalization(subImageLBP, maxgraylevel-mingraylevel,mingraylevel)
