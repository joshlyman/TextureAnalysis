...
outDir = '/Users/yanzhexu/Desktop/Research/EOR_Andrea/EOR_PI_featuresMap_from_Andrea/T2ROIoverT2PI/'
...
def drawColorMap(T1file,T1PIfile, caseoutDir,casefolder):
    ...
    for si in range(ttslicenum):
        ...
        plt.title(casefolder+' slice '+ str(si+1) + ' '+'T2 ROI overlap PI',fontsize=20)
        plt.savefig(caseoutDir + '/'+casefolder+' slice'+ str(si+1) + ' '+'T2 ROI overlap PI.png')
...
def drawTColormaps(dir,outDir):
    for casefolder in os.listdir(dir):
        ...
        T2ROImat = 'T2fill.mat'
        T2PI = casefolder + '_fromT2.mat'


        T2ROIfiledir = os.path.join(casefolderdir,T2ROImat)
        T2PIdir = os.path.join(casefolderdir,T2PI)
        ...
        drawColorMap(T2ROIfiledir,T2PIdir,caseoutDir, casefolder)
