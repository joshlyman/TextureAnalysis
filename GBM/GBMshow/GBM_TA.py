...
def Norm_Mean_Std_LargestBox2(subimage):
    ...
    # normalized to [0,255]
    subimage = (subimage - imgMin) * (255.0 / imgRange)

    meang = numpy.mean(subimage)
    stdg = numpy.std(subimage)
        ...
        for pt in ptnamelist: # such as RW,JTy...according to CSV file to sort
            ...
            for slicelist in ptslice:
                ...
                for eachdcm in dicomnames:
                    ...
                    dicommean, dicomstd= Norm_Mean_Std_LargestBox2(subImage)

                    subImageGabor = GrayScaleNormalization(subImage, subImage.max(),subImage.min())
